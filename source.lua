...
local prompt = useStudio and require(script.Parent.prompt) or loadWithTimeout('https://raw.githubusercontent.com/stewingit/stewfield/refs/heads/main/prompt.lua')
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("Stewfield | Function failed with error: ", result)
			return false
		else
			return result
		end
...
do
	...
	local assetFiles = {
		...
		["minimise"] = AssetBaseURL.."minimise.png?raw=true",
		["maximise"] = AssetBaseURL.."maximise.png?raw=true",
		["blank"] = AssetBaseURL.."blank.png?raw=true",
		["warning"] = AssetBaseURL.."warning.png?raw=true",
		["search"] = AssetBaseURL.."search.png?raw=true",
		["dot"] = AssetBaseURL.."dot.png?raw=true",
		["line"] = AssetBaseURL.."line.png?raw=true",
		["line2"] = AssetBaseURL.."line2.png?raw=true",
		...
		local ok, err = pcall(function()
			...
			for id, _ in assetFiles do
				...
				if success then
					...
					warn("Stewfield | Failed to load custom asset: "..tostring(id).." - "..tostring(asset))
				end
			end
		end)
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Stewfield | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Stewfield | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
function RayfieldLibrary:Notify(data)
	task.spawn(function()
		...
		if data.Actions then
			warn('Stewfield | Not seeing your actions in notifications?')
	...
	for _, tabbtn in ipairs(TabList:GetChildren()) do
		if tabbtn.ClassName == "Frame" and tabbtn.Name ~= "Placeholder" then
			...
			if targetPageName == tabbtn.Title.Text then
				...
				TweenService:Create(tabbtn.Title, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {TextTransparency = 0.2, TextColor3 = SelectedTheme.TabTextColor}):Play()
...
local function saveSettings()
	...
	for catName, category in pairs(settingsTable) do
		saveTable[catName] = {}
		for setName, setting in pairs(category) do
			saveTable[catName][setName] = { Value = setting.Value }
		end
	end
